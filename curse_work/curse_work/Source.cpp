...
void search(List<flightData> &flightlist, int num, string date) {
	for (int i = 0; i < flightlist.size(); i++) {
		if (flightlist[i].num == num && flightlist[i].date == date)
...
void main() {
	...
	if (fin) {
		while (fin) {
			...
			getline(fin, entry);
			if (entry == "") break;
			flightlist.Add(parseFlightData(entry));
		}
	}
	fin.close();
	...
	while (true) {
		...
		cout << "\n2 -> Удалить заявку";
		cout << "\n3 -> Все заявки";
		cout << "\n4 -> Поиск заявки по номеру рейса и дате вылета\n";
		int input;
		cin >> input;
		if (input == 0) break;
		switch (input) {
